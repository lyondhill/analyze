(function() {
  ...
  module.exports = ExpressServ = (function() {
    ...
    ExpressServ.prototype.quick_stats_hour = function(req, res) {
      return db.query("SELECT count(*) as \"total\", count(distinct pd) as \"unique\", avg(rt) as \"response\" FROM webrequest WHERE ai='4eb05aea48afd80192000057';", function(err, rows, moreResultSets) {
        if (err) {
          res.send(err);
        }
