(function() {
  ...
  module.exports = ExpressServ = (function() {
    ...
    ExpressServ.prototype.quick_stats_hour = function(req, res) {
      return db.query("SELECT COUNT(*) as total, COUNT(DISTINCT pd) as unique, avg(rt) as average FROM webrequest WHERE ai=" + req.params.app, function(err, rows, moreResultSets) {
        return res.send("total: " + rows[0].total + "\nunique: " + rows[0].unique + "\naverage: " + rows[0].average);
      });
    };
