(function() {
  ...
  module.exports = ExpressServ = (function() {
    ...
    ExpressServ.prototype.quick_stats_day = function(req, res) {
      return redis.hmget("" + req.params.app + "-quick_stats_day", function(err, response) {
        if (response) {
          return res.send(response);
        } else {
          return db.query("SELECT count(*) as \"total\", count(distinct pd) as \"unique\", avg(rt) as \"response\" FROM webrequest WHERE ai='" + req.params.app + "' and t>'2011-11-17'", function(err, result, moreResultSets) {
            res.send(result);
