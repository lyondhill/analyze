(function() {
  ...
  module.exports = ExpressServ = (function() {
    ...
    ExpressServ.prototype.quick_stats_day = function(req, res) {
      return db.query("SELECT count(*) as \"total\", count(distinct pd) as \"unique\", avg(rt) / 1000 as \"response\" FROM webrequest WHERE ai='" + req.params.app + "' and t>CURRENT_TIMESTAMP - INTERVAL '1' DAY", function(err, result, moreResultSets) {
        if (err) {
          console.log(err);
        }
        return res.send({
    ...
    ExpressServ.prototype.quick_stats_week = function(req, res) {
      return redis.get("" + req.params.app + "-quick_stats_week", function(err, response) {
        if (response) {
          ...
          return db.query("SELECT count(*) as \"total\", count(distinct pd) as \"unique\", avg(rt) / 1000 as \"response\" FROM webrequest WHERE ai='" + req.params.app + "' and t>CURRENT_TIMESTAMP - INTERVAL '7' DAY", function(err, result, moreResultSets) {
            var data;
            if (err) {
              console.log(err);
            }
    ...
    ExpressServ.prototype.quick_stats_month = function(req, res) {
      return redis.get("" + req.params.app + "-quick_stats_month", function(err, response) {
        if (response) {
          ...
          return db.query("SELECT count(*) as \"total\", count(distinct pd) as \"unique\", avg(rt) / 1000 as \"response\" FROM webrequest WHERE ai='" + req.params.app + "' and t>CURRENT_TIMESTAMP - INTERVAL '30' DAY", function(err, result, moreResultSets) {
            var data;
            if (err) {
              console.log(err);
            }
